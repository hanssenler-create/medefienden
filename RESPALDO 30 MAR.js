...
export default function MedefiendenHomepage() {
  const services = [
    ...
    },
    {
      ...
      description:
        "Atención online o presencial con coordinación dentro de 24 horas hábiles una vez confirmado el proceso.",
    },
    {
  ...
  const steps = [
    ...
    },
    {
      ...
      text: "Con claridad sobre el escenario, decides si avanzar con estrategia, gestión o litigación.",
    },
  ];
  ...
  <span className="text-xs tracking-wide text-neutral-400">
    Consilium ante, Actio post.
            ...
            <nav className="hidden gap-8 text-sm text-neutral-300 md:flex">
              ...
              <a href="#criterio" className="transition hover:text-white">Criterio</a>
          ...
          <div className="grid gap-14 py-16 lg:grid-cols-[1.15fr_0.85fr] lg:py-24">
            <div className="max-w-3xl">
              ...
              <p className="mt-6 max-w-2xl text-lg leading-8 text-neutral-300">
                Abogado con amplia experiencia en sector público y privado. Enfoque en materias civiles, administrativas y de familia. Cada caso se analiza estratégicamente desde el inicio, con reglas claras y tiempos definidos.
              </p>
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-3">
                {[
                  ...
                  <div key={a} className="rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur">
            ...
            <div className="lg:pl-6">
              <div className="rounded-[2rem] border border-white/10 bg-gradient-to-b from-white/10 to-white/5 p-6 shadow-2xl shadow-black/30 backdrop-blur">
                <div className="mb-6 flex items-center justify-between">
                  <div>
                    <div className="text-sm uppercase tracking-[0.3em] text-neutral-400">Perfil</div>
                    <div className="mt-2 text-2xl font-semibold text-white">Abogado fundador: Hans Senler Trapp</div>
  ...
  />
                    ...
                    <div className="mt-3 text-sm leading-7 text-neutral-300">
                      Amplia experiencia en sector público y privado, con enfoque en asuntos civiles, administrativos y de familia para personas. En el ambito de pequeñas empresas, ambito contractual y litigacion laboral
                    </div>
                  </div>
                  <div className="rounded-full border border-white/15 px-3 py-1 text-xs text-neutral-300">
                    Seriedad,compromiso y transparencia
                  </div>
                </div>
                <div className="rounded-3xl border border-white/10 bg-neutral-900/80 p-5">
                  <p className="text-sm leading-7 text-neutral-300">
                    Extendida experiencia en el sector público,  en municipalidades, ministerios e instituciones castrenses. Ejercicio independiente desde 2015, con más de mil causas tramitadas.
                  </p>
                </div>
      ...
      <section id="servicios" className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
        <div className="max-w-2xl">
          <div className="text-sm uppercase tracking-[0.3em] text-neutral-400">Servicios</div>
          <h2 className="mt-3 text-3xl font-semibold text-white md:text-4xl">
            Un servicio jurídico estructurado, claro y enfocado en resultados reales.
          </h2>
          <p className="mt-4 text-base leading-8 text-neutral-300">
            No buscamos clientes masivos, sino  un servicio personalizado. Seleccionamos bien los casos, explicamos con claridad y buscamos trabajar con criterio en materias civiles, administrativas y de familia, como tambien ser un valor agregado en Pequeñas y Medianas empresas.
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {services.map((service) => (
            ...
            >
              ...
              <p className="mt-4 text-sm leading-7 text-neutral-300">{service.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="proceso" className="border-y border-white/10 bg-white/[0.03]">
        <div className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
          <div className="flex max-w-3xl flex-col gap-4">
            <div className="text-sm uppercase tracking-[0.3em] text-neutral-400">Proceso</div>
            <h2 className="text-3xl font-semibold text-white md:text-4xl">
              Un proceso claro para entender qué estás contratando y qué puedes esperar.
            </h2>
            <p className="text-base leading-8 text-neutral-300">
              Desde el primer contacto, el objetivo es ordenar el caso, definir expectativas y evitar improvisaciones.
            </p>
          </div>

          <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {steps.map((step) => (
              <div key={step.n} className="rounded-[1.75rem] border border-white/10 bg-neutral-900/70 p-6">
                <div className="text-sm font-medium tracking-[0.25em] text-neutral-500">{step.n}</div>
                <div className="mt-4 text-xl font-semibold text-white">{step.title}</div>
                <p className="mt-3 text-sm leading-7 text-neutral-300">{step.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="criterio" className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
        <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="rounded-[2rem] border border-white/10 bg-white/5 p-8">
            <div className="text-sm uppercase tracking-[0.3em] text-neutral-400">Nuestro Compromiso</div>
            <h2 className="mt-3 text-2xl font-semibold text-white">Seriedad, compromiso y transparencia.</h2>
            <p className="mt-4 text-base leading-8 text-neutral-300">
              Una atención jurídica pensada para personas y pequeñas empresas que buscan claridad, orden y criterio, sin informalidad ni promesas irreales.
            </p>
            <div className="mt-6 rounded-3xl border border-white/10 bg-neutral-950/70 p-5 text-sm leading-7 text-neutral-300">
              “Aquí no se promete lo imposible. Se evalúa el caso, se explica con claridad y se actúa con estrategia.”
            </div>
          </div>

          <div className="rounded-[2rem] border border-white/10 bg-gradient-to-br from-blue-500/10 to-white/5 p-8">
            <div className="text-sm tracking-[0.3em] text-neutral-500">Nuestros Principios</div>
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              {[
                ["Cobros Justos: Recibes una oferta de negocios y un plan de pago"],
                ["Recibes una propuesta clara de lo que se ofrece hacer"],
                ["Conversación franca y directa con el profesional de tu caso"],
                ["Recibes un reporte vía telefono o mail cada vez que amerita"],
              ].map(([label, value]) => (
                <div key={label} className="rounded-2xl border border-white/10 bg-white/5 p-5">
                  <div className="text-xs tracking-[0.3em] text-neutral-500">{label}</div>
      ...
      <section id="contacto" className="border-t border-white/10 bg-neutral-900">
        <div className="mx-auto grid max-w-7xl gap-8 px-6 py-20 lg:grid-cols-[0.95fr_1.05fr] lg:px-10">
          <div>
            <div className="text-sm uppercase tracking-[0.3em] text-neutral-400">Contacto</div>
            <h2 className="mt-3 text-3xl font-semibold text-white md:text-4xl">
              Solicita una revisión inicial de tu caso.
            </h2>
            <p className="mt-4 max-w-xl text-base leading-8 text-neutral-300">
              Completa tus datos y describe brevemente el problema. La coordinación de la reunión se realiza dentro de 24 horas hábiles.
            </p>
          </div>

          <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-2xl shadow-black/20">
            <div className="grid gap-4 md:grid-cols-2">
              <input className="rounded-2xl border border-white/10 bg-neutral-950 px-4 py-3 text-sm text-white outline-none placeholder:text-neutral-500" placeholder="Nombre completo" />
              <input className="rounded-2xl border border-white/10 bg-neutral-950 px-4 py-3 text-sm text-white outline-none placeholder:text-neutral-500" placeholder="Correo electrónico" />
            </div>
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <input className="rounded-2xl border border-white/10 bg-neutral-950 px-4 py-3 text-sm text-white outline-none placeholder:text-neutral-500" placeholder="Teléfono" />
              <select className="rounded-2xl border border-white/10 bg-neutral-950 px-4 py-3 text-sm text-white outline-none">
                <option>Modalidad de reunión</option>
                <option>Online</option>
                <option>Presencial</option>
              </select>
            </div>
            <div className="mt-4">
              <select className="w-full rounded-2xl border border-white/10 bg-neutral-950 px-4 py-3 text-sm text-white outline-none">
                <option>Área o tipo de problema</option>
                <option>Civil</option>
            ...
            <div className="mt-4 rounded-2xl border border-blue-400/20 bg-blue-400/10 p-4 text-sm leading-7 text-blue-100">
              Se coordinará la reunión dentro de 24 horas hábiles tras la revisión del formulario.
            </div>
            <button className="mt-5 w-full rounded-2xl bg-white px-6 py-3 text-sm font-medium text-neutral-950 transition hover:-translate-y-0.5">
              Enviar solicitud
            </button>
          </div>
        </div>
